...
* On the large datasets, the models chosen by the sample splitting with 50% and 70% values on the train, choose higher values of k than the ones that the 30% Sample splitting does. The same thing happens also for CV-5 and CV-10: both metrics prefer an higher number of gaussians. We didn't expect this result, but by looking further into it and from the fact the 30% train-70% test gets values closer to 7 we realized that it's probably a consequence of only using the log-likelihood as a metric without penalizing the number of parameters used in the model. We also found out, by doing some other tests that having larger test set in general leads to slightly better results but not close to the "ideal" one.
