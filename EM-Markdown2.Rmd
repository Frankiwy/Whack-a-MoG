...
* On the large datasets, the models chosen by the sample splitting (especially with 50% and 70% as the train set) ancross validation methodusually go for high number of gaussians. We were at first confused by this, but it's  since one is only using the log-likelihood as a metric and it doesn't penalize the number of parameters used in the model. These models with more than 6 gaussians are not overfitting (in the sense of "not being predictive on unseen data") and a lot of the extra gaussians in the trained model have small p values or are somewhat reduntant (some gaussians can be almost obtained by multiplying a gaussian by a number)
...
single_hist <- function(matrix_to_convert,sample_s,legend_location="none"){
  ...
  p <- df %>%
    ...
    xlab(paste("Small Sample n = ",sample_s,sep="")) +
    theme_ipsum() +
    theme(
...
grid.arrange(single_hist(small_first_matrix,300),
             single_hist(big_first_matrix,5000, legend_location = "rigth"), nrow=1, ncol=2,
             top = textGrob("TOP 1st Results",gp=gpar(fontsize=15,font="bold")))
```

```{r include=TRUE, echo=FALSE, warning=FALSE}
grid.arrange(single_hist(small_matrix,300),
             single_hist(big_matrix,5000, legend_location = "right"), nrow=1, ncol=2,
