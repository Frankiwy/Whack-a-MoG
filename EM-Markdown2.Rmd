...
On the other hand, on large sample sizes (n=5000), all the methods have a lower variance even though the average is not necessarily close to 6 but is usually large. As an example look at all the CV results.
...
* The k value chosen by the AIC is always higher or equal than the one chosen from the BIC for both sample sizes. This is expected since the penalization that the BIC gives to the number of parameters used is higher and the fit for small datasets is not very good. The effect is still there for the large sample size, but it's less visible and the number of chosen gaussians is often the same. AIC, BIC and SamS_30 are the three metrics that return results closer to the "ideal" one (k=6) on large sample sizes.

* On the large datasets, the models chosen by the sample splitting with 50% and 70% values on the train, choose higher values of k than the ones that the 30% Sample splitting does. The same thing happens also for CV-5 and CV-10: both metrics prefer an higher number of gaussians. We didn't expect this result, but by looking further into it we realized that in general a lot of the gaussians have very small p in models with a lot of them. One would expect these methods to find the "ideal" model because a large number of gaussian can overfit more easily (and this happens with small sample sizes), but this doesn't happen in our tests with our k_max and these models actually get, on average, slighly better results. We also found out, by doing some other tests that having an even larger size in the dataset leads to slightly better results but still not close to the "ideal" one. 
